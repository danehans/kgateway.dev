Use the {{< reuse "/docs/snippets/kgateway.md" >}} control plane to deploy and manage the lifecycle of ingress2gateway proxies on Kubernetes. </br></br>

{{< reuse "docs/snippets/ingress2gateway/about.md" >}}
...
{{< reuse "docs/snippets/ingress2gateway/get-started.md" >}}

Good job! You now have the {{< reuse "/docs/snippets/kgateway.md" >}} control plane running in your cluster.

## Next steps

{{< icon "ingress2gateway" >}} [Create an ingress2gateway proxy]({{< link-hextra path="/setup/" >}}) that you can use for Model Context Protocol (MCP), agent-to-agent (A2A), large language model (LLM), and more AI-related use cases. For example, you can follow the [guide]({{< link-hextra path="/mcp/static-mcp/" >}}) to use ingress2gateway to proxy traffic to a sample MCP tool server. The example deploys a sample MCP server with a `fetch` tool, exposes the tool with ingress2gateway, and tests the tool with the MCP Inspector UI.
